...
from lexicon.lexicon_ru import button_main_menu
...
def get_menu_kb(*, level: int, size: tuple[int] = (2,)):
    ...
    for text, menu_name in button.items():
...
def get_user_catalog_btns(*, level: int, categories: list, sizes: tuple[int] = (2,)):
    ...
    for c in categories:
        keyboard.add(InlineKeyboardButton(text=c.name,
                callback_data=CallbackMenu(level=level+1, menu_name=c.name, category=c.id).pack()))
    ...
    *,
    ...
    if page:
        ...
        InlineKeyboardButton(text='Заказать',
                    callback_data=CallbackMenu(level=0, menu_name='order').pack()),
        ]
